...
class DoubleDQN:
    ...
    def selectDoubleGreedyActionSoft(self, netA, netB, frames):
        outputsA = self.generatePredictions(netA, frames)[0]
        outputsB = self.generatePredictions(netB, frames)[0]
    ...
    def update(self):
        ...
        targetsA = np.zeros((MINIBATCH_SIZE/2, NUMBER_ACTIONS))
        targetsB = np.zeros((MINIBATCH_SIZE/2, NUMBER_ACTIONS))
        real_filterA = np.zeros((MINIBATCH_SIZE/2, NUMBER_ACTIONS))
        real_filterB = np.zeros((MINIBATCH_SIZE/2, NUMBER_ACTIONS))
        ...
        for i in range(len(actions)):
            if i < MINIBATCH_SIZE/2:
                real_filterA[i][int(actions[i])] = 1.0
                if next_frames[i][0][0][0] == -1:
                    targetsA[i, int(actions[i])] = rewards[i]
                else:
                    qa = q_valuesApast[i][int(actions[i])]
                    qb = q_valuesBnext[i][astar]
                    targetsA[i, int(actions[i])] = qa + (rewards[i] + GAMMA * qb - qa)
            else:
                j = i % (MINIBATCH_SIZE/2)
                real_filterB[j][int(actions[i])] = 1.0
                if next_frames[j][0][0][0] == -1:
                    targetsB[j, int(actions[i])] = rewards[i]
                else:
                    qa = q_valuesAnext[i][bstar]
                    qb = q_valuesBpast[i][int(actions[i])]
                    targetsB[j, int(actions[i])] = qb + (rewards[i] + GAMMA * qa - qb)

        self.netA.fit([real_filterA, past_frames[:MINIBATCH_SIZE/2]], targetsA, batch_size=MINIBATCH_SIZE/2, nb_epoch=1, verbose=0)
        self.netB.fit([real_filterB, past_frames[MINIBATCH_SIZE/2:]], targetsB, batch_size=MINIBATCH_SIZE/2, nb_epoch=1, verbose=0)
